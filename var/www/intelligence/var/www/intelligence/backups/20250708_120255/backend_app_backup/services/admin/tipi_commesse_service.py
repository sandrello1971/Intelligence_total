...
class TipiCommesseService:
    ...
    def is_in_use(self, tipo_commessa_id: str) -> bool:
        ...
        try:
            ...
            opportunity_count = self.db.execute(text("""
                SELECT COUNT(*) FROM opportunities 
                WHERE tipo_commessa_id = :tipo_commessa_id
            """), {"tipo_commessa_id": tipo_commessa_id}).scalar()
            
            return (milestone_count or 0) > 0 or (opportunity_count or 0) > 0
        except Exception as e:
        ...
        try:
            ...
            most_used_query = self.db.execute(text("""
                SELECT tc.id, tc.nome, tc.codice, 
                       COALESCE(m.milestone_count, 0) + COALESCE(o.opportunity_count, 0) as usage_count
                FROM sub_types tc
                LEFT JOIN (
                ...
                LEFT JOIN (
                    SELECT tipo_commessa_id, COUNT(*) as opportunity_count  
                    FROM opportunities
                    GROUP BY tipo_commessa_id
